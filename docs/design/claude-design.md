# Claude Design System — SPONGECLUB-2 공식 디자인 가이드

> 이 문서는 SPONGECLUB-2에서 만드는 모든 UI(대시보드·페이지·카드 등)에 적용되는 디자인 기준이다.
> 핵심 3요소: **크림 캔버스(#faf9f5) + 코랄 액센트(#cc785c) + 다크 네이비 제품 표면(#181715)**. 헤드라인은 슬랩-세리프(Copernicus / 대체: Tiempos·EB Garamond), 본문은 휴머니스트 산세(StyreneB / 대체: Inter).
