...
- **구현한 것 (링크·스크린샷 — 이미지는 `이미지첨부/` 폴더에):**
  ...
  - **마스코트**: 실제 반려견(허스키) 사진을 레퍼런스로 생성형 이미지 AI(STIC 프롬프트)로 제작한 오리지널 캐릭터. 6가지 포즈(인사·가리키기·만세·달리기·졸림·기본)가 메뉴마다 등장하고, 시간대·완료 여부에 따라 자동 전환
...
**구현 형태:** 자체 완결형 웹앱(HTML) 프로토타입 **고고버디(GoGo Buddy)** — 여고생이 매일 열고 싶은 귀여운 파스텔 UI, 반려견 기반 오리지널 마스코트가 메뉴마다 다른 포즈로 반응. (이미지 6종을 파일에 내장해 링크 하나로 배포)
