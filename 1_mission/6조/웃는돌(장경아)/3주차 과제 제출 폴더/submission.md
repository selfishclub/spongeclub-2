...
- **(크루/토크데이에서 받은 피드백 반영)** '나만의 OS 유닛'을 통해, 내 데이터가 "텔레그램(메시지 통과) → Vercel(내 앱 서버) → Anthropic(Claude가 분류·회고·초안) → Supabase(최종 저장)"처럼 여러 서버를 지나고 있다는 점이 염려된다고 검토의견을 요청했고, **'로컬 대시보드 vs. 공개 웹 서비스' 구분**을 제안받아 조만간 업그레이드할 예정이다.
...
- '뉴스레터 글감 - 역할 전환 비용'을 넣으니, **서로 다른 역할의 흔적 8개**(엄마 가족 돌봄 일정·캠프 선택·기념일 예약 / 본업 팀장 보고·부서 미팅 / 사이드 …)를 **근거로 묶어** 초안을 지어 줌.
- 결과: 브런치 초안 **"엄마에서 팀장으로, 그 3분의 거리"** — 뼈대(5줄)와 오프닝 문단까지. *가족 돌봄 전화를 끊자마자 팀장 보고 자료를 여는 3분*을 '역할 전환 비용'이라는 내 글감과 실제로 엮어냈다.
...
- **"작동하는 코드"와 "실제 돌아가는 서비스"는 완전히 새로운 Challenge!!!** 로직은 2주차에 이미 됐지만, 배포·환경변수·webhook·보안버전 같은 '연결의 벽' 을 하나씩 넘는 게 진짜 완성이었다. 눈에 안 보이는 이 마지막 구간이 제일 많은 삽질을 요구했다.
- **막히면 조각내서 확인.** webhook 401을 만났을 때 전체를 의심하지 않고 getMe로 토큰만 격리 테스트해 원인을 5초 만에 갈랐다. 모든 "안 돼요"는 더 작은 "이 조각이 안 돼요"로 조각내어 확인할 수 있다.
- **이제 흔적 OS는 '기획'도 '스킬'도 아니고, 매일 텔레그램에 던지면 저절로 쌓이는 나의 실제 인프라!** 2주차엔 손으로 커밋하던 걸, 3주차엔 세상 어디서든 열리는 URL로 만들었다. - "내가 돌보는 시스템"에서 "나를 돌보는 시스템"으로.
- **바이브 코딩은 기대치를 끌어올린다.** 자연어로 함께 만들다 보니 "되게만 하기"에서 멈추지 않고 자꾸 욕심을 내게 된다. v1.0 배포 이후에도 회고 자동화 → 글감 스튜디오 → UI 리프레시로 계속 밀 수 있었던 건, 만드는 문턱이 낮아진 만큼 원하는 기준(디자인 가이드라인·상용 서비스 수준)으로 더 높게 잡게 됐기 때문이다. 선택과 집중이 필요한 시점이다.
- **내 기록이 콘텐츠 IP가 된다.** 글감 스튜디오를 붙이며 깨달은 것 — 흘려보낸 흔적은 소비되고 끝나는 메모가 아니라, 되살려 채널별 글로 발행할 수 있는 원재료였다. 캡처의 목적이 '정리'에서 '생산'으로 한 칸 넓어졌다. 다만, 아직 각 채널의 특성이나 별도로 분산되어 관리하고 있던 데이터를 어떻게 한곳에 모을지가 고민이다.
