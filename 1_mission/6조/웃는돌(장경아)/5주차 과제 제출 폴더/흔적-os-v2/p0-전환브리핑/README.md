...
> ⚠️ **2026-07-30 갱신.** 아래 기준은 v2.1(A=본캐) 시절 것이라 폐기됐다.
> **현행 게이트 지표·판정표는 [`../05_측정설계.md`](../05_측정설계.md) 가 유일한 기준이다.**

~~저녁 승인 주 4일 이상 · 아침 완료율 주 3일 이상~~ → **보조 지표로 강등.**
이 둘은 A(역할 전환 비용 제거)를 재는 지표였고, 본캐가 B′로 바뀌면서 판정 기준에서 빠졌다.

**P0의 성패는 이제 이것으로 잰다:**
- [ ] 저녁·아침 메시지 **7일 연속** 도착
- [ ] **주당 신규 흔적 10개 이상 · 3개 이상 역할** (= corpus 정지선 도달)
- [ ] 주간 회고에 **지표 4줄이 자동으로 찍힌다** (측정 자동화)

> **W0에서 만들 것:** `05_측정설계.md` §1의 테이블 4개(`user_activity`·`studio_entry_log`·`publication`·`weekly_metrics`)와 일요일 스냅샷 cron.
> `daily_focus`의 `source`(auto/approved)·`done`은 계속 쌓되, **게이트 판정이 아니라 corpus 건강도**로 읽는다.
