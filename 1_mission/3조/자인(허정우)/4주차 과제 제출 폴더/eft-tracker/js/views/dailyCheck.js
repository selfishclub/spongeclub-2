...
import { SITUATION_EXAMPLES, BODY_LOCATION_EXAMPLES } from '../data.js';
import { createEmotionPicker } from '../emotionPicker.js';

// 자가평가 = 평가 Lv.1: 상황 → 감정 모두 → 주된 감정 → 강도 → 몸의 위치
export function renderDailyCheck(root, ctx) {
  ...
  function render() {
    container.innerHTML = '';
    container.appendChild(el('div', { class: 'stepbar', text: `평가 Lv.1 · ${step} / ${TOTAL}` }));
  ...
  function controls() {
    ...
    if (step < TOTAL) {
      ...
      row.appendChild(el('button', { class: 'primary', onClick: save, text: '평가 저장' }));
    }
    return row;
  }
    ...
    switch (step) {
      ...
      case 2: {
        const picker = createEmotionPicker({
          multi: true,
          initial: draft.emotions,
          onChange: (sel) => { draft.emotions = sel; },
        });
        return el('div', { class: 'card' },
      ...
      case 3: {
        const card = el('div', { class: 'card' },
          el('h2', { text: '3. 그중 가장 주된 감정 하나를 고르세요' }),
        );
        if (draft.emotions.length === 0) {
          card.appendChild(el('p', { class: 'empty', text: '2단계에서 감정을 먼저 골라주세요.' }));
          return card;
        }
        const chips = el('div', { class: 'chips' }, ...draft.emotions.map((emotion) => {
          const b = el('button', { class: 'chip' + (draft.chosenEmotion === emotion ? ' on' : ''), type: 'button', text: emotion });
          b.addEventListener('click', () => { draft.chosenEmotion = emotion; render(); });
          return b;
        }));
  ...
  function save() {
    ...
    container.appendChild(el('div', { class: 'card result' },
      ...
      el('div', { class: 'row' },
        el('button', { class: 'primary', onClick: () => ctx.navigate('session'), text: '이어서 EFT 실천' }),
        el('button', { class: 'ghost', onClick: () => ctx.navigate('dashboard'), text: '홈으로' }),
      ),
    ));
  }
}
