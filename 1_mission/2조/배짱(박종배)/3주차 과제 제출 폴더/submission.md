...
- **결과물 (링크·스크린샷 — 이미지는 `이미지첨부/` 폴더에):**
  ...
  **📷 말씀 사진관 — 현장에서 인쇄한 카드**
  ...
  | 어르신 | 담긴 말씀 |
  |---|---|
  | 이의복 어르신 | 내게 능력 주시는 자 안에서 내가 모든 것을 할 수 있느니라 (빌립보서 4:13) |
  | 남순자 어머님 | 하나님을 사랑하는 자에게는 모든 것이 합력하여 선을 이루느니라 (로마서 8:28) |
  | 최학순 어머님 | 네 길을 여호와께 맡기라 그를 의지하면 그가 이루시고 (시편 37:5) |
  | 임숙자 어머님 | 너희는 여호와의 선하심을 맛보아 알지어다 (시편 34:8) |
  | 남순자 어머님 | 네 영혼이 잘됨 같이 네가 범사에 잘되고 강건하기를 (요한삼서 1:2) |
  | 황순분 어머님 | 감사함으로 그의 문에 들어가며 찬송함으로 그의 궁정에 들어가서 (시편 100:4) |
  | 박수열 어머님 | 네가 내 눈에 보배롭고 존귀하며 내가 너를 사랑하였은즉 (이사야 43:4) |
  | 박봉선 어머님 | 여호와는 나의 목자시니 내게 부족함이 없으리로다 (시편 23:1) |
  | 손달순 어머님 | 여호와께서 그의 얼굴을 네게 비추사 은혜 베푸시기를 원하며 (민수기 6:25) |
  | 황순옥 어머님 | 아무 것도 염려하지 말고 모든 일에 기도와 간구로 감사함으로 하나님께 아뢰라 (빌립보서 4:6) |

  > 인쇄 카드 실물 사진은 `이미지첨부/말씀사진관/` 폴더에 첨부 (10장 이상 제작).
