...
> 사실 이게 내 **메인 OS**다. (앞의 두 OS도 여기서 쓰는 도구로 만들었다.) 핵심은 **긴 강의 영상을 세로 쇼츠 '편집 재료'로 자동 변환**하는 것.

### 🎯 뭘 하는 OS인가
- 두 번째(슬랙 영상 OS)가 "0에서 새 영상을 만드는" 쪽이라면, 이건 **이미 있는 긴 강의를 잘라 쇼츠로 바꾸는** 편집 자동화다.
- **"이 강의 쇼츠로 만들어줘"** 한 줄이면 → 콘티(설계) → 컷 → 자막 → 타이포·효과 → 완성까지 알아서 굴러간다.
- 원칙: **자막은 영상에 굽지 않고 SRT로 따로** 뽑는다. 그래야 프리미어·캡컷 등에서 자유롭게 스타일을 입힌다 — "완성본"이 아니라 **"편집기로 바로 넘길 재료"**를 만드는 게 목표라서.
- 덤: 같은 재료에 **스타일만 바꿔** 여러 버전을 뽑는다. 사람이 하나씩 손으로 안 만들어도 된다.

**실제로 이 OS가 뽑아준 완성 쇼츠 (영상 · 나레이션 포함):**
...
![한 줄 지시 → 쇼츠 파이프라인](이미지첨부-영상편집/다이어그램2_파이프라인.png)
  1. **콘티(기획자)** — 먼저 "팔리는 쇼츠 한 편"의 설계도부터(훅·반전·숫자 증거·엔딩).
  2. **컷·구간(탐색가)** — 긴 영상에서 쓸 구간만 찾아 이어붙이기(무음·딴 얘기 컷).
  3. **자막·타이포(실행가)** — 자막(SRT) 뽑고, 세로 비율로 바꾸고, 타이포·효과음·배속 얹어 합치기.
  4. **검수(검수자)** — 자막 싱크·오타 확인, 틀리면 되돌리기.
...
### 🔧 어떻게 작동하고, 어떻게 고치나
- 핵심 아이디어: **쇼츠 하나 = 매니페스트(설계표) JSON 한 장.** 이 한 장에 그 쇼츠에 필요한 걸 다 적어둔다.
```json
{
  "id": 22,
  "segments": [[3609.7, 3621.6], [3630.9, 3650.7]],
  "speaker": null,
  "speed": 1.0,
  "cues": [
    {"text": "다 공유하면 손해?\n**정반대였습니다**",
     "kind": "title", "pos": "top", "from": 0.5, "to": 31.6}
  ]
}
```
  → `segments`(원본에서 쓸 구간·초) · `speaker`(발화자 얼굴, 없으면 null) · `speed`(배속) · `cues`(언제 `from~to` / 무슨 자막·타이포 `kind` / 어디 `pos`에 띄울지).
- 그러면 **빌드 명령 한 줄**(`07_build.py`)이 이 표를 읽어 **컷 → 자막(SRT) → 타이포 → 효과음 → 완성**까지 자동으로 뽑아준다. 손으로 하나씩 안 돌려도 된다 — 그래서 이 표를 **"재현의 원천"**이라 부른다.
- **고칠 때도 코드는 안 건드린다:**
  - 세로(9:16) ↔ 가로(16:9)? → 설정 파일(`config.yaml`)의 `aspect` **한 줄만** 바꿈.
  - 어느 구간 자르고 어떤 문구 띄울지? → 매니페스트 JSON의 숫자·문장만 고쳐 **다시 빌드.**
  - 자막은 아예 영상에 안 굽고 **SRT로 따로** 뽑아, 프리미어·캡컷 어디서든 스타일 자유.
- 이 구조라 **같은 재료로 버전 여러 개를 빠르게 비교**하며 다듬었다 (텍스트 크기 줄이기 → 나레이션 트림 → 여백 조정 = v1~v4).
![이 자동화가 '배선'된 구조 — 클로드코드가 도구·일꾼을 지휘한다](이미지첨부-영상편집/다이어그램1_OS구조.png)

### ✨ 세 줄 요약
- 강의를 쇼츠로 바꿀 때 **매번 반복되던 컷·자막·타이포·배속**을 자동화한 게 이 OS다.
- 비결은 **쇼츠 하나를 매니페스트 JSON 한 장으로** 적어두는 것 — 코드 안 건드리고 값만 바꿔 **똑같이 재현·수정**한다.
- 자막은 안 굽고 SRT로 빼서, 완성본이 아니라 **어느 편집기로든 바로 쓰는 '재료'**를 만든다.
